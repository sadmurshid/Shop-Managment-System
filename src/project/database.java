...
public class database {
    ...
    private static ResultSet resultSet, rs;
...
//                    System.out.println("add new invoice : "+sql);
    ...
    protected String[][] getFullInventoryWOBuyPrice(String user){
        try{
            String [][]products= new String[][]{{null, null}, {null, null}};
            if(checkAccess(user, "inventory_access")){
                sql="USE "+databaseName;
                statement.executeUpdate(sql);
                sql="SELECT product_ID, product_name, company_name,  quantity, sell_price, sr_name, minimum_require FROM inventory ;";
                resultSet=statement.executeQuery(sql);
                resultSet.last();
                int size=resultSet.getRow();
                resultSet.beforeFirst();
                ...
                int i=0;
                while(resultSet.next()) {
                    products[i][0] = ""+resultSet.getInt("product_ID");
                    products[i][1] = ""+resultSet.getString("product_name");
                    products[i][2] = ""+resultSet.getString("company_name");
                    products[i][3] = ""+resultSet.getDouble("quantity");
                    products[i][4] = ""+resultSet.getDouble("sell_price");
                    products[i][5] = ""+resultSet.getString("sr_name");
                    products[i][6] = ""+resultSet.getDouble("minimum_require");
